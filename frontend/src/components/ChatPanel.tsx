...
function EngineWakingBanner({ engine }: { engine: EngineState }) {
  ...
  return (
    <div className="rounded-xl border border-rule bg-paper-raised px-4 py-3 flex items-center gap-3">
...
function IngestStages({ ticker, companyName, phase }: { ticker: string; companyName?: string | null; phase: IngestPhase }) {
  ...
  return (
    <div className="rounded-xl border border-rule bg-paper-raised px-[18px] py-4 max-w-[560px]">
  ...
  return (
    <div className="flex-1 flex flex-col bg-paper min-w-0">
      ...
      <div className="flex-1 overflow-y-auto px-8 py-7">
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center min-h-[60vh] text-center gap-5">
            <div className="w-14 h-14 rounded-2xl bg-paper-raised border border-rule flex items-center justify-center">
              {ticker ? <Building2 className="w-6 h-6 text-accent-ink" /> : <MessageSquare className="w-6 h-6 text-ink-soft" />}
            </div>
            {!ticker ? (
              <>
                <div>
                  <h3 className="text-base font-semibold text-ink">Ask about any public company</h3>
                ...
                <div className="flex flex-wrap gap-2 justify-center max-w-sm">
                  {[["AAPL", "Apple"], ["MSFT", "Microsoft"], ["NVDA", "NVIDIA"], ["AMZN", "Amazon"]].map(([t, name]) => (
                    <button key={t} onClick={() => selectCompany(t, name)}
                      className="text-xs px-3 py-1.5 rounded-full border border-rule hover:border-accent-ink text-ink-soft hover:text-ink transition-all bg-paper-raised">
                      {name}
                    </button>
                  ))}
                </div>
          ...
          if (msg.role === "user") {
            return (
              <div key={msg.id} className="flex justify-end">
                <div className="max-w-[80%] rounded-[14px] px-4 py-3 text-sm leading-relaxed bg-accent-ink-soft border border-accent-ink text-ink">
          ...
          return (
            <div key={msg.id} className="flex flex-col gap-2.5">
              ...
              <div className="flex items-center gap-2">
                <ThesisMark size={22} />
                <span className="text-xs font-semibold text-ink">Thesis</span>
              ...
              {msg.citations && msg.citations.length > 0 && (
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  {msg.citations.map((c, i) => {
                    const chipClass = "flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-paper-raised border border-rule text-[11px] text-ink hover:border-accent-ink hover:text-ink transition-all max-w-56";
                ...
                <div className="flex flex-wrap gap-1.5 pt-0.5">
                  ...
                  {!followUps && !loadingSuggestions[msg.id] && (
                    <button
                      ...
                      className="text-[11px] text-ink-soft hover:text-accent-ink border border-rule hover:border-accent-ink px-2.5 py-1 rounded-full transition-all"
                    >
                      + Suggest follow-ups
                    </button>
                  )}
                  {(followUps ?? []).map(s => (
                    <button
                      key={s}
                      onClick={() => onSend(s)}
                      className="text-[11.5px] text-ink hover:text-ink bg-paper-raised hover:bg-paper-raised border border-rule hover:border-accent-ink px-3.5 py-[7px] rounded-full transition-all"
                    >
                      {s}
                    </button>
                  ))}
        ...
        {isLoading && (
          <div className="flex flex-col gap-2.5">
            ...
            {ingestPhase === "ingesting" || ingestPhase === "polling" ? (
              ...
              <div className="rounded-xl border border-rule bg-paper-raised px-4 py-3 flex items-center gap-2 self-start">
      ...
      <form onSubmit={handleSubmit} className="border-t border-rule px-8 pt-3.5 pb-3 bg-paper flex flex-col items-center">
        <div className="w-full max-w-[780px] rounded-[14px] border border-rule bg-paper-raised p-3 space-y-2.5">
          <div className="flex items-center justify-between gap-2">
            {/* Company pill */}
            {ticker && !pickerOpen ? (
              <div className="flex items-center gap-2 min-w-0">
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-accent-ink-soft border border-accent-ink text-xs min-w-0">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <div className="flex border border-rule rounded-full p-0.5 bg-paper">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2.5 bg-accent-ink hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed rounded-[9px] transition-all active:scale-95"
            >
